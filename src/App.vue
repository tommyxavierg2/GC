  ...
  <div id="app">
    <v-app>
      ...
      <v-toolbar>
        ...
        <v-spacer></v-spacer>
          <v-btn class="hidden-sm-and-down" flat v-for="(item, index) in $t('message.toolbarItems')" :key="index" :to="item.route" @click="active = index">
            {{item.name}}
          </v-btn>
      </v-toolbar>
      <v-navigation-drawer v-model="drawer" absolute temporary app>
        ...
        <v-list class="pa-1 mt-3">
          <v-list-tile v-for="(item, index) in $t('message.toolbarItems')" :key="index" :to="item.route">
            <v-list-tile-avatar>
